//-----------------------------------------------------------------
// 功能：从资源文件加载D2D位图，更多详细解释请参考：http://www.cnblogs.com/Ray1024/
// 作者：Ray1024
// 网址：http://www.cnblogs.com/Ray1024/
//-----------------------------------------------------------------

#include "D2DCreateBitmapFromResource.h"
#include "resource.h"
...
HRESULT DemoApp::Initialize()
{
    ...
    if (SUCCEEDED(hr))
    {
        ...
        m_hwnd = CreateWindow(
            L"D2DDemoApp",
            L"D2DCreateBitmapFromResource",
            WS_OVERLAPPEDWINDOW,
            CW_USEDEFAULT,
            CW_USEDEFAULT,
            static_cast<UINT>(ceil(480.f * dpiX / 96.f)),
            static_cast<UINT>(ceil(360.f * dpiY / 96.f)),
            NULL,
            NULL,
            HINST_THISCOMPONENT,
            this
...
HRESULT DemoApp::CreateDeviceResources()
{
    ...
    if (!m_pRT)
    {
		...
		if (SUCCEEDED(hr))
		{
			LoadResourceBitmap(m_pRT,m_pWICFactory,MAKEINTRESOURCE(IDB_PNG1),L"PNG",0,0, &m_pBitmap);
		}
    }
